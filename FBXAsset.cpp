...
bool FBXAsset::LoadAsset(const std::string& Filename)
{
	...
	FbxGlobalSettings& setting = m_scene->GetGlobalSettings();
	TRACE("setting.GetOriginalUpAxis()=%d\n", setting.GetOriginalUpAxis());
	...
	for (auto index = 0; index < AnimStackNameArray.GetCount(); index++)
	{
		...
		for (auto itr = (*tree).Begin(); itr; itr = itr->next)
		{
			...
			if (rotCurve)
			{
				...
				ASSERT(quatCurve);
				//auto Vec3CurveToQuatCurve = [](AnimCurveQuat* dst, AnimCurveVec3* src)
				//	{
				//		dst->name = src->name;
				...
				MyMath::Vec3CurveToQuatCurve(quatCurve, rotCurve);
...
bool FBXAsset::ParseMesh(NodeTree::Item* item)
{
	...
	if (item->data.flags & ATTRIBUTE_FLAG_MESH)
	{
		...
		TRACE("fbxsdk;GeometricTranslation=%s\n", glm::to_string(ToGlmVec3(GeometricTranslation)).c_str());
		TRACE("fbxsdk;GeometricTranslation=%s\n", glm::to_string(ToGlmVec3(GeometricRotation)).c_str());
		TRACE("fbxsdk;GeometricTranslation=%s\n", glm::to_string(ToGlmVec3(GeometricScaling)).c_str());
		m->GeometryOffset = ToMyMat(GeometryOffset);
		TRACE("fbxsdk;GeometryOffset=%s\n", glm::to_string(m->GeometryOffset).c_str());
...
bool FBXAsset::ParseMeshCP(MeshInfo* meshinfo)
{
	...
	for (auto i = 0; i < pointsCount; i++)
	{
		...
		meshinfo->cp_[i].ResetBoneWeight();
...
#pragma region BoneWeight
	TRACE("-------------------------------------------\n");
	{
		...
		if (skinDeformer)
		{
			...
			//fbxsdk::FbxAMatrix GeometryOffset(GeometricTranslation, GeometricRotation, GeometricScaling);
			//meshinfo->GeometryOffset = ToMyMat(GeometryOffset);
			TRACE("fbxsdk;GeometryOffset=%s\n", glm::to_string(meshinfo->GeometryOffset).c_str());
			...
			for (int clusterIndex = 0; clusterIndex < clusterCount; clusterIndex++)
			{
				...
				ci.SetTransformMatrix(ToMyMat(clusterTransform)* meshinfo->GeometryOffset);
				...
				PAUSE(link != nullptr);
				TRACE_QUIET("bonename=%s\n", link->GetName());
				TRACE_QUIET("TransformMatrix=%s\n", glm::to_string(ToMyMat(clusterTransform)).c_str());
				TRACE_QUIET("TransformLinkMatrix=%s\n", glm::to_string(ToMyMat(clusterTransformLink)).c_str());
				...
				{
					auto w = weightsPtr[i];
					if (!meshinfo->cp_[indicesPtr[i]].AddBoneWeight(clusterIndex, w))
					{
				...
				for (int i = 0; i < pointCount; i++)
				{
					meshinfo->cp_[indicesPtr[i]].SortBoneWeight();
				}
				//			TRACE("%s\n", ci.ToString().c_str());
	...
	*/
	...
	for (auto fi = 0; fi < faceCount; fi++)
	{
		...
		VertexPNCTAW newVtx[3] = {
			VertexPNCTAW(cp0.pos, cp0.normal, cp0.color, vec2(0.0f), vec3(0.0f), cp0.bone, cp0.weight),
			VertexPNCTAW(cp1.pos, cp1.normal, cp1.color, vec2(0.0f), vec3(0.0f), cp1.bone, cp1.weight),
			VertexPNCTAW(cp2.pos, cp2.normal, cp2.color, vec2(0.0f), vec3(0.0f), cp2.bone, cp2.weight),
		};
		// ポリゴン毎のマテリアル割り当て
		int materialIndex = 0;
			...
			if (uvMappingMode == FbxGeometryElement::eByPolygonVertex)
			{
				...
				for (auto vidx = 0; vidx < polygonSize; vidx++)
				{
					...
					if (uvReferenceMode == FbxGeometryElement::eDirect)
					{
						...
						uv[1] = 1.0 - uv[1];
						newVtx[vidx].texcoord = ToGlmVec2(uv);
					}
...
#pragma region TANGENT
	...
	// マッピングモードとリファレンスモードを確認
	{
		...
		for (auto i = 0; i < meshinfo->ibuf_.size(); i += 3)
		{
			...
			auto& uv0 = meshinfo->vbuf_[i0].texcoord;
			auto& uv1 = meshinfo->vbuf_[i1].texcoord;
			auto& uv2 = meshinfo->vbuf_[i2].texcoord;
...
void FBXAsset::RenderCP(std::vector<VertexPNW>* pnw)
{
	for (auto m : m_meshes)
	{
		for (auto i = 0; i < m->ibuf_.size(); i += 3)
		{
			...
			auto& b = m->deform_[ib];
			auto& c = m->deform_[ic];
			pnw->push_back(VertexPNW(a.pos, a.normal));
			pnw->push_back(VertexPNW(b.pos, b.normal));
			pnw->push_back(VertexPNW(c.pos, c.normal));
		}
	}
}
...
void FBXAsset::BuildNodeTree(NodeTree* tree, uint32_t NodeID, fbxsdk::FbxNode* node, NodeTree::Item* parent)
{
	...
	TRACE("%s:rotOrder=%d\n", node->GetName(), node->RotationOrder.Get());
...
void FBXAsset::UpdateSamplePoint(float time)
{
	...
	{
		auto& ni = itr->data;
		if (std::holds_alternative<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclScaling]))
		{
			auto lclScaling = std::get<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclScaling]);
			m_samplePoint.answers[ni.lclScaling].xyz = lclScaling->Evaluate(time);
		}
		else
		{
			m_samplePoint.answers[ni.lclScaling].xyz = vec3(1.0f);
		}
		if (std::holds_alternative<AnimCurveQuat*>(m_currentDopeSheet->curves[ni.lclRotation]))
		{
			auto lclRotation = std::get<AnimCurveQuat*>(m_currentDopeSheet->curves[ni.lclRotation]);
			m_samplePoint.answers[ni.lclRotation].q = lclRotation->Evaluate(time);
		}
		else
		{
			m_samplePoint.answers[ni.lclRotation].q = quat();
		}
		if (std::holds_alternative<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclTranslation]))
		{
			auto lclTranslation = std::get<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclTranslation]);
			m_samplePoint.answers[ni.lclTranslation].xyz = lclTranslation->Evaluate(time);
		}
		else
		{
			m_samplePoint.answers[ni.lclTranslation].xyz = vec3(0.0f);
		}
	}
	if (m_sampleAlpha > 0.0f)
	{
...
mat4 FBXAsset::CalcLocalTransform(NodeTree::Item* item)
{
	...
	if (m_currentDopeSheet->curves.count(ni.lclScaling))
	{
		if (std::holds_alternative<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclScaling]))
		{
			lclScaling = std::get<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclScaling]);
		}
	}
	if (m_currentDopeSheet->curves.count(ni.lclRotation))
	{
		if (std::holds_alternative<AnimCurveQuat*>(m_currentDopeSheet->curves[ni.lclRotation]))
		{
			lclRotation = std::get<AnimCurveQuat*>(m_currentDopeSheet->curves[ni.lclRotation]);
		}
	}
	if (m_currentDopeSheet->curves.count(ni.lclTranslation))
	{
		if (std::holds_alternative<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclTranslation]))
		{
			lclTranslation = std::get<AnimCurveVec3*>(m_currentDopeSheet->curves[ni.lclTranslation]);
		}
	}
	vec3 scaling(1.0f);
	quat rotation;
...
//			auto& inVertex = mesh->cp_[i];
			...
			mesh->deform_[i].texcoord = inVertex.texcoord;
