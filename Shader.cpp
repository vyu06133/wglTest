...
{
	TRACE("CompileShaderFromFile(%s,\"%s\")\n", glEnumMap[type], file);
	path_ = file;
	std::vector<uint8_t> buf;
#if 1//ネストしないinclude対応
	HRESULT hr = S_OK;
...
#endif
	buf.push_back(0);// NULLターミネート分を追加
	auto bufdata = (char*)buf.data();
	puts((char*)bufdata);
	OutputDebugStringA((char*)bufdata);
	OutputDebugStringA("\n");
	if (FAILED(hr))
	{
		ASSERT(0);
...
bool Shader::CompileShaderFromMemory(GLenum type, const std::vector<uint8_t>& src)
{
	...
	glCompileShader(shader_);
	{
		...
		TRACE("COMPILE STATUS:%s\n", glEnumMap[status]);
		if (status == GL_FALSE)
		{
			GLsizei size;
			glGetShaderiv(shader_, GL_INFO_LOG_LENGTH, &size);
			TRACE("COMPILE LOG LENGTH:'%d'\n", size);
