...
Texture2D* Texture2D::CreateFromMemory(const void* data, size_t size)
{
	Texture2D* result = nullptr;
	result = _NEW Texture2D;
	if (result == nullptr)
	...
	if (!result->LoadMemory(data, size))
	{
		return nullptr;
	}
	return result;
...
bool Texture2D::LoadMemory(const void* data, size_t size)
{
	int w, h, bpp;
	GLenum format = GL_RED;
	uint8_t* pixels = stbi_load_from_memory((stbi_uc const*)data, (int)size, &w, &h, &bpp, 0);
	...
	//TexSubImage(w, h, GL_RGBA, pixels);
#endif
