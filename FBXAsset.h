...
public:
	...
	struct ClusterInfo
	{
		...
		std::string ToString() const
		{
			...
			ss << "Transform " << glm::to_string(ClusterTransform) << std::endl;
	...
	using AnimCurveVariant = std::variant<AnimCurveVec3*, AnimCurveQuat*>;
	...
	struct DopeSheet
	{
		...
		float stop_ = 0.0f;
#if 1
		std::unordered_map<std::string, AnimCurveVariant> curves;// TODO:型安全性を考慮すべき
#else
	...
	void RenderCP(std::vector<VertexPNW>* pnw);
