...
#pragma region MinMaxAbs
	...
	inline vec3 Max(const vec3& s, const vec3& t)
	{
		return glm::max(s, t);
	}
	inline vec3 Max(const quat& s, const quat& t)
	{
		return glm::max(glm::make_vec4(&s.w), glm::make_vec4(&t.w));
	}
	...
	inline vec3 Min(const vec3& s, const vec3& t)
	{
		return glm::min(s, t);
	}
...
#pragma region AnimCurve
	...
	struct AnimCurve
	{
		...
		std::vector<KeyFrame> keys;
		AnimCurve() = default;
		AnimCurve(const std::string& n, const T& d):name(n),defaultValue(d){}
		void Init()
		{
			keys.clear();
		};
		...
		const T SetDefaultValue(const T& v)
		{
			defaultValue = v;
			return defaultValue;
		}
		...
		const T SetCurrentValue(const T& v)
		{
			currentValue = v;
			return currentValue;
		}
		...
		T Evaluate(const float& time)
		{
			...
			for (auto i = 0u; i < keyCount - 1; i++)
			{
				...
				{
					float t = (time - key1.first) / (key2.first - key1.first);
					return SetCurrentValue(Interp(key1.second, key2.second, t));
				}
			}
			return SetCurrentValue(defaultValue);
		}
	...
	inline bool Vec3CurveToQuatCurve(OUT AnimCurve<quat>* dst, IN AnimCurve<vec3>* src)
	{
...
#pragma endregion
	...
	public:
		...
		inline static Rect XYWH(long x, long y, uint32_t w, uint32_t h)
		{
			Rect rc;
			rc.left = x;
			rc.top = y;
